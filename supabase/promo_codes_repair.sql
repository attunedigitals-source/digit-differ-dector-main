-- PROMO CODES & PAID USER TRACKING SETUP / REPAIR SCRIPT
-- Run this script directly in the Supabase SQL Editor if needed.

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'profiles' AND column_name = 'has_ever_paid'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'payments' AND column_name = 'discount_code'
    ) THEN
...
CREATE TABLE IF NOT EXISTS public.promo_codes (
  ...
  scope TEXT NOT NULL DEFAULT 'trial_only',
